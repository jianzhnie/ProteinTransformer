...
class ProteinConfig(object):
    """Base class for all configuration classes. Handles a few parameters
    common to all models' configurations as well as methods for
    ...
    def save_pretrained(self, save_directory):
        """Save a configuration object to the directory `save_directory`, so
        that it can be re-loaded using the
        :func:`~ProteinConfig.from_pretrained` class method."""
...
def prune_linear_layer(layer, index, dim=0):
    ...
    Return the pruned layer as a new layer with requires_grad=True. Used to remove heads.
    """
...
class MLMHead(nn.Module):
    ...
    def forward(self, hidden_states, targets=None):
        ...
        if targets is not None:
            ...
            masked_lm_loss = loss_fct(hidden_states.view(-1, self.vocab_size),
                                      targets.view(-1))
...
class SequenceToSequenceClassificationHead(nn.Module):
    ...
    def forward(self, sequence_output, targets=None):
        ...
        if targets is not None:
            ...
            metrics = {
                'accuracy':
                acc_fct(sequence_logits.view(-1, self.num_labels),
                        targets.view(-1))
            }
            loss_and_metrics = (classification_loss, metrics)
            outputs = (loss_and_metrics, ) + outputs
...
class PairwiseContactPredictionHead(nn.Module):
    def __init__(self, hidden_size: int, ignore_index=-100):
        super().__init__()
        self.predict = nn.Sequential(nn.Dropout(),
    ...
    def forward(self, inputs, sequence_lengths, targets=None):
        ...
        prediction = prediction[:, 1:-1,
                                1:-1].contiguous()  # remove start/stop tokens
        outputs = (prediction, )
        ...
        if targets is not None:
            ...
            metrics = {
                'precision_at_l5':
                self.compute_precision_at_l5(sequence_lengths, prediction,
                                             targets)
            }
