"""Utilities for working with the local dataset cache. This file is adapted
from the huggingface transformers library at
https://github.com/huggingface/transformers, which in turn is adapted from the
AllenNLP library at https://github.com/allenai/allennlp Copyright by the
AllenNLP authors.
...
def cached_path(url_or_filename, force_download=False, cache_dir=None):
    ...
    determine which. If it's a URL, download the file and cache it, and return
    the path to the cached file. If it's already a local path, make sure the
    file exists and then return the path.
    ...
    if parsed.scheme in ('http', 'https', 's3'):
        ...
        output_path = get_from_cache(url_or_filename, cache_dir,
                                     force_download)
...
def get_from_cache(url,
    ...
    with FileLock(lock_path):
        ...
        with temp_file_manager() as temp_file:
            logger.info(
                '%s not in cache or force_download=True, download to %s', url,
                temp_file.name)
