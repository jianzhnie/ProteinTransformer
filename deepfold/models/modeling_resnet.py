...
class ProteinResNetBlock(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.conv1 = MaskedConv1d(in_channels=config.hidden_size,
                                  out_channels=config.hidden_size,
                                  kernel_size=3,
                                  padding=1,
                                  bias=False)
        # self.bn1 = nn.BatchNorm1d(config.hidden_size)
        self.layernorm1 = ProteinResNetLayerNorm(config)
        self.conv2 = MaskedConv1d(in_channels=config.hidden_size,
                                  out_channels=config.hidden_size,
                                  kernel_size=3,
                                  padding=1,
                                  bias=False)
        # self.bn2 = nn.BatchNorm1d(config.hidden_size)
        self.layernorm2 = ProteinResNetLayerNorm(config)
...
class ProteinResNetEmbeddings(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        position_ids = torch.arange(seq_length - 1,
                                    -1,
                                    -1.0,
...
class ProteinResNetForMaskedLM(ProteinResNetAbstractModel):
    def __init__(self, config):
        ...
        self.mlm = MLMHead(hidden_size=config.hidden_size,
...
class ProteinResNetForSequenceToSequenceClassification(
