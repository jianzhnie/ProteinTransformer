...
from .models.utils.modeling_utils import ProteinModel
...
class Registry:
    ...
    def register_task(cls,
        ...
        if dataset is not None:
            ...
            return lambda dataset: cls.register_task(task_name, num_labels,
                                                     dataset, models)

    @classmethod
    def register_task_spec(cls,
                           task_name: str,
                           task_spec: Optional[TAPETaskSpec] = None):
        """Registers a task_spec directly.

        If you find it easier to actually create a TAPETaskSpec manually, and then register it, feel free to use this method, but otherwise it is likely easier
        to use `registry.register_task`.
        """
        if task_spec is not None:
            if task_name in cls.task_name_mapping:
                raise KeyError(
                    f"A task with name '{task_name}' is already registered")
            cls.task_name_mapping[task_name] = task_spec
            return task_spec
        else:
            return lambda task_spec: cls.register_task_spec(
                task_name, task_spec)
    ...
    def register_task_model(cls,
        ...
        return cls.task_name_mapping[task_name].register_model(
    ...
    def get_task_model(cls,
        ...
        """Create a TAPE task model, either from scratch or from a pretrained
        model.
